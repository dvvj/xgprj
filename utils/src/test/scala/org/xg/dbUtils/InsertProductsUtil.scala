...
object InsertProductsUtil extends App {
  ...
  val products = (1 to 4).map { idx =>
    val (name, price0, keywords, categories, details) = prodInfoMap(idx)
    MProduct(
      idx, name, price0, MProdDetail.toJson(details), keywords, categories, AssetsNA
    )
  }.toArray

  val insertStatementTemplate =
    "INSERT INTO products (id, name, price0, detailed_info, keywords, categories)" +
      "  VALUES (%s);"
